  ...
  return (
    ...
    >
      <button type="button" className="preview-lightbox-close" onClick={onClose} aria-label="关闭">
        <AppIcon name="ri:close-line" className="size-6" />
      </button>
      <img
        src={src}
        alt="预览"
  ...
  useEffect(() => {
    ...
    async function render() {
      ...
      try {
        ...
        if (!cancelled) {
          setHtml(`<p class="text-destructive">渲染 Markdown 失败</p>`);
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  if (!content.trim()) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 py-12 text-muted-foreground">
        <AppIcon name="ri:file-text-line" className="size-10 opacity-50" />
