...
export function MediaLibraryPanel() {
  ...
  const [mediaPendingDeletion, setMediaPendingDeletion] = useState<MediaFile | null>(null);
  ...
  const handleDelete = async () => {
    if (!mediaPendingDeletion) return;
    const file = mediaPendingDeletion;
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmActionDialog
        open={Boolean(mediaPendingDeletion)}
        onOpenChange={(open) => {
          if (!open && !deletingPath) setMediaPendingDeletion(null);
        }}
        title={`永久删除“${mediaPendingDeletion?.name || ''}”？`}
        description="图片会从 GitHub 仓库删除。Cloudflare Pages 部署后，网站中使用这张图片的位置可能无法显示；后台无法恢复。"
        confirmLabel="永久删除图片"
        pending={Boolean(deletingPath)}
        destructive
