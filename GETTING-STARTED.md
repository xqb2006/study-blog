...
### Cloudflare Pages 免费部署（推荐）

1. Fork 仓库到你的 GitHub 账号。
2. 在 Cloudflare Pages 中选择该 GitHub 仓库。
3. 构建命令填写 `npm run build`。
4. 输出目录填写 `dist`。
5. 等待构建完成并打开平台提供的 `pages.dev` 地址。

部署完成后，将 `config/site.yaml` 中的 `site.url` 改成实际地址，再提交一次即可。

### 网页写作

根目录的 `.pages.yml` 已配置文章字段和图片目录。连接 GitHub 后，可以通过 Git 型网页 CMS 新建文章、编辑 Markdown 和上传图片；文章最终仍保存为 GitHub 仓库中的 Markdown 文件。
