...
export const onRequestPost = async (context: any) => {
  try {
    ...
    return json({ success: true, file: { name: filename, publicPath: path.replace(/^public/, ''), relativePath: path.slice('public/img/'.length), size: file.size, modifiedAt: new Date().toISOString(), extension: filename.split('.').pop()?.toLowerCase() ?? '' } });
  } catch (error) {
