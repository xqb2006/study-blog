...
export const onRequestPost = async (context: any) => {
  try {
    ...
    await putFile(context, toPostPath(post.id), makeMarkdown({ ...post.frontmatter, sticky }, post.content), `cms: ${sticky ? '置顶' : '取消置顶'} ${post.id}`, post.file.sha);
    return json({ success: true, sticky, buildSync: { success: true, status: 'pending' } });
  } catch (error) {
