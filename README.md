...
> 本发行版在保留原主题外观与功能的基础上，增加了零费用部署和 Git 型网页写作方案。个人配置请从 `config/site.yaml` 开始，网页 CMS 配置见 `.pages.yml`。
...
推荐使用 **Cloudflare Pages** 免费部署静态博客，也支持 Vercel、Netlify 和 Docker 等方式。零费用方案不需要数据库、服务器或付费 API。
