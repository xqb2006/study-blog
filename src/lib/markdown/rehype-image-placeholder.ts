...
function shouldLoadEagerly(node: Element): boolean {
  const src = node.properties?.src;
  return typeof src === 'string' && src.startsWith('/img/yuque/relay-stations/');
}
  ...
  return (tree: Root) => {
    visit(tree, 'element', (node: Element, index, parent) => {
      ...
      if (parent.type === 'element' && parent.tagName === 'a') {
        node.properties = {
          ...node.properties,
          loading: shouldLoadEagerly(node) ? 'eager' : 'lazy',
          decoding: 'async',
        };
        return;
      }
